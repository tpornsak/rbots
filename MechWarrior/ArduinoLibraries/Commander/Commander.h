...
{    
  public:
    ...
    int goalAzPos;    // Azimuth turret low byte position
    int goalElPos;    // Elevation turret low byte position
        
    // Hooks are used as callbacks for button presses -- NOT IMPLEMENT YET
        
  private:
    // internal variables used for reading messages
    unsigned char vals[11];  // temporary values, moved after we confirm checksum
